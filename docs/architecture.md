...
**1. Quota and rate-limit control.** The two paths differ here, and the argument is strongest for the fallback loop. In the fallback loop, concurrency defeats early stop: calls already launched cannot be un-launched even when the first provider alone would have sufficed, so it is strictly more billed calls (Brave ~1,000/mo, Exa ~2,800/mo, Tavily credits, Jina RPM/tokens). In the T1 hot path the call count is fixed by `T1_PREMIUM_COUNT`, so concurrency would not add calls; what it would destroy is the inter-call spacing that acts as de-facto pacing against per-second and per-minute caps (Brave QPS, Jina 500 RPM), which raises 429 risk in both paths. A single 4xx from any provider trips that engine's circuit breaker for 5 minutes — an availability cost, not just a metric blip.
